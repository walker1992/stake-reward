...
keys_dir=keys
stake_reward=../../../target/debug/stake-reward
...
setup_pool () {
  ...
  $stake_reward create-pool --epoch-fee-numerator 3 --epoch-fee-denominator 100 \
...
add_validator_stakes () {
  ...
  do
    $stake_reward add-validator $pool $validator
  done
}
